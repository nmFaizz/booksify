...
const BookButtons = (props: {buyLinks: any}) => {
    ...
    return (
        <div className="flex gap-4 items-center">
            <div>
                <button 
                    className="bg-stone-200 rounded-sm flex items-center h-8 relative" 
                    onClick={() => setIsClicked(prev => !prev)}
                >
                    <p className="px-4">Buy Links</p>
                    <RiArrowDropDownLine 
                        style={{display: 'inline', fontSize: '24px'}} 
                        className="bg-stone-400 h-full hover:bg-stone-500 rounded-e-sm" 
                    /> 
            ...
            <button className="bg-stone-200 rounded-sm px-4 h-8">Add</button>
            <button className="w-[40px] h-[40px] rounded-full flex items-center justify-center bg-stone-200">
